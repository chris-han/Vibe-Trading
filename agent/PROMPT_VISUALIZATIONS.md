...
This file lists locations in the `agent/` tree that contain prompts, code, or generated output which produce diagrams, tables, or charts (Mermaid, VChart, Markdown tables, Rich tables, printed charts, etc.). Use the links to jump to the relevant file/lines.

- **Session rendering rules**: [agent/src/session/service.py](agent/src/session/service.py#L58-L96) — guidance to render tables as Markdown, flowcharts as Mermaid (```mermaid```), and charts as VChart (```vchart```).
...
- The scan focused on explicit keywords (Mermaid, VChart, chart/table/plot, Rich tables, pandas DataFrame and prompt YAML). There may be additional places that render visuals indirectly (e.g., functions producing JSON intended for VChart, or skills that return Markdown). If you want, I can:
...
If you'd like more detail (code snippets, extracted Mermaid/VChart blocks, or a CSV summary), tell me which format you prefer.
...
| Charts (VChart JSON) | [agent/src/session/service.py](agent/src/session/service.py#L66-L96); frontend expects ```vchart``` blocks | Prompt (VChart JSON) → frontend VChart (JS); validated in Python service |
