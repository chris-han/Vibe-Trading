...
- ✅ Emoji-led Markdown bullets for entry, add, reduce, stop-loss, and target-price instructions
- 📋 A Markdown pipe-table such as `Tier | Price Zone | Trigger | Action | Risk Note`

Make Markdown the default for this section. Use bullets or a pipe-table instead of a standalone visual layout.
...
- any `操作策略` / action-plan section is formatted as emoji-led bullets or a Markdown pipe-table, never as an ASCII or box-drawing graphic
...
8. **Action-plan formatting**: recommendation and `操作策略` sections must stay in normal Markdown. Never use plain fenced code blocks, ASCII cards, or terminal box-drawing layouts for trading steps.
