...
def test_session_workflow_prompts_do_not_encode_file_or_dir_operations():
    ...
    assert "Never use terminal ls/cd commands against the /workspace upload alias" in session_service._DOCUMENT_WORKFLOW_PROMPT
...
def test_swarm_workflow_hints_do_not_encode_file_or_dir_operations():
    ...
    combined = "\n".join(
        [
            swarm_worker.BACKTEST_WORKFLOW_PROMPT,
            swarm_worker.DOCUMENT_WORKFLOW_PROMPT,
            swarm_worker.MARKET_DATA_WORKFLOW_PROMPT,
        ]
    )
    ...
    assert "current workspace upload area" in swarm_worker.DOCUMENT_WORKFLOW_PROMPT
    assert "Never use terminal ls/cd commands against the /workspace upload alias" in swarm_worker.DOCUMENT_WORKFLOW_PROMPT
    assert "Never search Desktop, Downloads, /mnt" in swarm_worker.DOCUMENT_WORKFLOW_PROMPT
...
def test_session_and_swarm_runtime_prompts_share_common_policy_source():
    ...
    assert swarm_worker.BACKTEST_WORKFLOW_PROMPT == runtime_prompt_policy.BACKTEST_WORKFLOW_PROMPT
    assert swarm_worker.DOCUMENT_WORKFLOW_PROMPT == runtime_prompt_policy.DOCUMENT_WORKFLOW_PROMPT
    assert swarm_worker.MARKET_DATA_WORKFLOW_PROMPT == runtime_prompt_policy.MARKET_DATA_WORKFLOW_PROMPT